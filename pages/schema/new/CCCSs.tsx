...
const getResistorForm = (normal: number, freq: number) => {
    ...
    for(let j = 0; j < normal; j++) {
        form[`mFirstNode${j}`] = {
            ...createFormFieldConfig(`Первый отрицательный узел ИТУТ №${j}`, `mFirstNode${j}`,'number'),
            validationRules: [
        ...
        form[`pFirstNode${j}`] = {
            ...createFormFieldConfig(`Первый положительный узел ИТУТ${j}, кОм`, `pFirstNode${j}`,'number'),
            validationRules: [
        ...
        form[`mSecondNode${j}`] = {
            ...createFormFieldConfig(`Второй отрицательный узел ИТУТ №${j}`, `mSecondNode${j}`,'number'),
            validationRules: [
        ...
        form[`pSecondNode${j}`] = {
            ...createFormFieldConfig(`Второй положительный узел ИТУТ${j}, кОм`, `pSecondNode${j}`,'number'),
            validationRules: [
        ...
        form[`transmission${j}`] = {
            ...createFormFieldConfig(`Проводимость передачи ИТУТ${j}, кОм`, `transmission${j}`,'number'),
            validationRules: [
    ...
    for(let i = normal; i < normal + freq; i++) {
        form[`mFirstNode${i}`] = {
            ...createFormFieldConfig(`Первый отрицательный узел ИТУТ №${i}`, `mFirstNode${i}`,'number'),
            validationRules: [
        ...
        form[`pFirstNode${i}`] = {
            ...createFormFieldConfig(`Первый положительный узел ИТУТ${i}, кОм`, `pFirstNode${i}`,'number'),
            validationRules: [
        ...
        form[`mSecondNode${i}`] = {
            ...createFormFieldConfig(`Второй отрицательный узел ИТУТ №${i}`, `mSecondNode${i}`,'number'),
            validationRules: [
        ...
        form[`pSecondNode${i}`] = {
            ...createFormFieldConfig(`Второй положительный узел ИТУТ${i}, кОм`, `pSecondNode${i}`,'number'),
            validationRules: [
        ...
        form[`T1${i}`] = {
            ...createFormFieldConfig(`Постоянная времени T1 ИТУТ №${i}`, `T1${i}`,'number'),
            validationRules: [
                isEmptyRule(`T1${i}`),
                minNumRule(`T1${i}`, 0),
                maxNumRule(`T1${i}`, 100)
...
const CREATE_CCCSS_MUTATION = gql`
    mutation createCCCSs(
        $schemaId: Int!
...
const CCCSs = () => {
    ...
    let str = router.query.CCCSs;
    const CCCSs = (str! as string).split('+').map(elem => +elem);
    return (
        <>
            <MyForm router={router} elemName={'CCCSs'} name='ИТУТ схемы' elemForm={getResistorForm(CCCSs[0],CCCSs[1])} mutation={CREATE_CCCSS_MUTATION} getData={getGraphQLData} />
        </>
    );
};
