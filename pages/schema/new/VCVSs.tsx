...
const getResistorForm = (normal: number, freq: number) => {
    ...
    for(let j = 0; j < normal; j++) {
        form[`mFirstNode${j}`] = {
            ...createFormFieldConfig(`Первый отрицательный узел ИНУН №${j}`, `mFirstNode${j}`,'number'),
            validationRules: [
        ...
        form[`pFirstNode${j}`] = {
            ...createFormFieldConfig(`Первый положительный узел ИНУН${j}, кОм`, `pFirstNode${j}`,'number'),
            validationRules: [
        ...
        form[`mSecondNode${j}`] = {
            ...createFormFieldConfig(`Второй отрицательный узел ИНУН №${j}`, `mSecondNode${j}`,'number'),
            validationRules: [
        ...
        form[`pSecondNode${j}`] = {
            ...createFormFieldConfig(`Второй положительный узел ИНУН${j}, кОм`, `pSecondNode${j}`,'number'),
            validationRules: [
        ...
        form[`transmission${j}`] = {
            ...createFormFieldConfig(`Проводимость передачи ИНУН${j}, кОм`, `transmission${j}`,'number'),
            validationRules: [
    ...
    for(let i = normal; i < normal + freq; i++) {
        form[`mFirstNode${i}`] = {
            ...createFormFieldConfig(`Первый отрицательный узел ИНУН №${i}`, `mFirstNode${i}`,'number'),
            validationRules: [
        ...
        form[`pFirstNode${i}`] = {
            ...createFormFieldConfig(`Первый положительный узел ИНУН${i}, кОм`, `pFirstNode${i}`,'number'),
            validationRules: [
        ...
        form[`mSecondNode${i}`] = {
            ...createFormFieldConfig(`Второй отрицательный узел ИНУН №${i}`, `mSecondNode${i}`,'number'),
            validationRules: [
        ...
        form[`pSecondNode${i}`] = {
            ...createFormFieldConfig(`Второй положительный узел ИНУН${i}, кОм`, `pSecondNode${i}`,'number'),
            validationRules: [
        ...
        form[`T1${i}`] = {
            ...createFormFieldConfig(`Постоянная времени T1 ИНУН №${i}`, `T1${i}`,'number'),
            validationRules: [
                isEmptyRule(`T1${i}`),
                minNumRule(`T1${i}`, 0),
                maxNumRule(`T1${i}`, 100)
        ...
        form[`T2${i}`] = {
            ...createFormFieldConfig(`Постоянная времени T2 ИНУН${i}, кОм`, `T2${i}`,'number'),
            validationRules: [
                isEmptyRule(`T2${i}`),
                minNumRule(`T2${i}`, 0),
                maxNumRule(`T2${i}`, 100)
        ...
        form[`transmission${i}`] = {
            ...createFormFieldConfig(`Проводимость передачи ИНУН${i}, кОм`, `transmission${i}`,'number'),
            validationRules: [
...
const CREATE_VCVCS_MUTATION = gql`
    mutation createVCVSs(
        $schemaId: Int!
...
const VCVSs = () => {
    ...
    let str = router.query.VCVSs;
    const VCVSs = (str! as string).split('+').map(elem => +elem)
    return (
        <>
            <MyForm router={router} elemName={'VCVSs'} name='ИНУН схемы' elemForm={getResistorForm(VCVSs[0],VCVSs[1])} mutation={CREATE_VCVCS_MUTATION} getData={getGraphQLData} />
        </>
    );
};
